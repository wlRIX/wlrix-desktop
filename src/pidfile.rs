...
//! A pidfile, so `desktop.toml` can be reloaded without hunting for the process.
//!
//! `SIGHUP` re-reads the config (see [`crate::signals`]), and the point of that is being able to
//! change the icon size or the terminal command without restarting the desktop -- which needs a
//! pid, and a sibling process cannot read one from the environment. So it goes in a well-known
//! file under the per-user runtime directory, the same arrangement `wlrix-compositor` and
//! `wlrix-idle` use, and the one `wlrix-settings-daemon` looks for.
//!
...
//! -- which is what [`crate::session`] already does when reading the compositor's.

use std::path::PathBuf;

/// Named for the process, beside the compositor's and the idle manager's.
const PID_NAME: &str = "wlrix-desktop.pid";
...
pub fn write() -> Option<Guard> {
    ...
    match create(&path).and_then(|mut file| {
        ...
        Ok(()) => Some(Guard { path }),
        Err(err) => {
            eprintln!("wlrix-desktop: could not write {}: {err}", path.display());
            None
        }
    }
}
...
/// Removes the pidfile on drop, so a live pidfile means a live desktop.
pub struct Guard {
    path: PathBuf,
}
...
impl Drop for Guard {
    fn drop(&mut self) {
        if let Err(err) = std::fs::remove_file(&self.path) {
            eprintln!(
                "wlrix-desktop: could not remove {}: {err}",
                self.path.display()
            );
        }
    }
...
mod tests {
    ...
    fn the_pidfile_sits_beside_the_others() {
        ...
        // looks for it. A change here that is not mirrored there would make every desktop.toml
        // setting silently stop applying live.
        assert!(path().ends_with("wlrix-desktop.pid"));
    }
}
