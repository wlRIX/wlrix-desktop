...
The palette comes from [`wlrix-ui`](https://github.com/wlRIX/wlrix-ui), which is generated by
`wlrix-assets/tools/palettegen` from the shared palette JSON — the same source the compositor, the greeter and the
Avalonia theme are built from. Edit the JSON and re-run `just palette` from `wlrix-epoch`; never edit the generated
file.

Which scheme is drawn comes from `desktop.toml`:
...
`wlrix-ui` also supplies the canvas, the Motif bevel, the fonts, the coverage mask behind the icons and the SVG/PNG
decoder — all of which this repo used to carry its own copy of, kept in step with `wlrix-greeter` by hand.

It is a **git dependency pinned by rev**, not a path dependency, so this repo still builds from a fresh standalone
clone. To work on both sides at once, `just link-ui` in `wlrix-epoch` writes a `[patch]` into `.cargo/config.toml`
pointing at a sibling checkout, and `just bump-ui` moves the pin once the change is committed there.

What stayed here: the icon *shapes* (`src/ui/icons.rs`), because what a folder looks like is this component's business,
and the `Icon=` name lookup (`src/icon_theme.rs`), because that is the icon-theme spec and it is this component that
reads `.desktop` files.
